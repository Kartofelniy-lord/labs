...
int BSearch2(int value, int* startArray, int left, int right) {
	bool flag = false;
	int mid;
...
int BSearch2(int arr[], int value, int left, int right) { // (arr масив в котором ищем, s то что ищем, r размер масива)
	int m = (left + right) / 2;
	if (arr[m] == value) {
		cout << "Число найдено" << endl;
		return m;
	...
	if (arr[m] < value) BSearch2(arr, value, m, right - 1); else BSearch2(arr, value, left, m - 1);
}
